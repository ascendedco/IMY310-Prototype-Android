...
public class Seeding {
    ...
    public Seeding() {
       ...
       /* ArrayList<Item> orderOne = new ArrayList<>();
        orderOne.add(items.get(0).clone());
        orderOne.add(items.get(5).clone());
        orderOne.add(items.get(9).clone());
        orderOne.add(items.get(7).clone());
        for (Item i: orderOne) { i.setQuantity(1); }
        orders.add(new Order(0, tables.get(0), orderOne, 0, "Ready"));

        ArrayList<Item> orderTwo = new ArrayList<>();
        orderOne.add(items.get(2).clone());
        orderOne.add(items.get(10).clone());
        orderOne.add(items.get(10).clone());
        orderOne.add(items.get(6).clone());
        for (Item i: orderTwo) { i.setQuantity(1); }
        orders.add(new Order(1, tables.get(0), orderTwo, 0, "Delayed"));

        ArrayList<Item> orderThree = new ArrayList<>();
        orderOne.add(items.get(3).clone());
        orderOne.add(items.get(11).clone());
        orderOne.add(items.get(13).clone());
        orderOne.add(items.get(14).clone());
        for (Item i: orderThree) { i.setQuantity(1); }
        orders.add(new Order(2, tables.get(0), orderThree, 0, "Ready"));*/

        Database.INSTANCE.seed(tables, categories, items);
    }
}
